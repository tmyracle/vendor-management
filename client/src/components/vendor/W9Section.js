...
const W9Section = (props) => {
  ...
  const handleVendorUpdate = async () => {
    try {
      const res = await axios.patch(
        ...
        { w9_required: !props.vendor.w9_required },
        withToken()
      );
      if (res.status === 200) {
        toast.success("W9 requirement updated");
  ...
  const handleEditClick = () => {
    props.toggleW9EditModal(props.vendor.w9s[0]);
  };
    ...
    >
      <div className="bg-white shadow border border-gray-200 sm:rounded-lg">
        <div className="px-4 py-5 sm:px-6">
          ...
          <Switch
            checked={props.vendor.w9_required}
            onChange={handleVendorUpdate}
            className={classNames(
              props.vendor.w9_required ? "bg-green-600" : "bg-gray-200",
          ...
          >
            ...
            <span
              aria-hidden="true"
              className={classNames(
                props.vendor.w9_required ? "translate-x-5" : "translate-x-0",
                "pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"
              )}
            />
          </Switch>
        </div>
        {props.vendor.w9_required ? (
          <div className="border-t border-gray-200">
            {props.vendor.w9s && props.vendor.w9s.length >= 1 ? (
              <div className="relative grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2 rounded-lg bg-white px-6 py-5 shadow-sm">
                {props.vendor.w9s[0].document_name ? (
                  <div className="p-3 col-span-2 flex items-center justify-between text-sm border border-gray-200 rounded-md">
                    ...
                    <div className="ml-4 flex-shrink-0">
                      <a
                        href={props.vendor.w9s[0].document_url}
                        target="_blank"
                        rel="noreferrer"
                ...
                <div className="sm:col-span-1">
                  ...
                  <dd className="mt-1 text-sm text-gray-900">
                    {props.vendor.w9s[0].taxpayer_id_number}
                  </dd>
                </div>
                <div className="sm:col-span-1">
                  <dt className="text-sm font-medium text-gray-500">
                ...
                <div className="sm:col-span-1">
                  ...
                  <dd className="mt-1 text-sm text-gray-900">
                    {formatDate(props.vendor.w9s[0].updated_at)}
                  </dd>
                </div>
              </div>
            ) : (
              <div className="text-center py-5">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Get started by adding a W9.
                </p>
                <div className="mt-6">
                  <button
                    type="button"
                    onClick={props.toggleW9AddModal}
